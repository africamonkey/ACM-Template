...
\begin{itemize}
\item len[i]表示编号为i的节点表示的回文串的长度（一个节点表示一个回文串）
\item next[i][c]表示编号为i的节点表示的回文串在两边添加字符c以后变成的回文串的编号（和字典树类似）。
\item fail[i]表示节点i失配以后跳转不等于自身的节点i表示的回文串的最长后缀回文串（和AC自动机类似）。
\item cnt[i]表示节点i表示的本质不同的串的个数（建树时求出的不是完全的，最后count()函数跑一遍以后才是正确的）
\item num[i]表示以节点i表示的最长回文串的最右端点为回文串结尾的回文串个数。
\item last指向新添加一个字母后所形成的最长回文串表示的节点。
\item st[i]表示第i次添加的字符（一开始设st[0] = -1（可以是任意一个在串S中不会出现的字符））。
\item tot 表示添加的节点个数。
\item n 表示添加的字符个数。
\end{itemize}
...
struct PAM {
	int n, tot, last;
	int len[N], fail[N], next[N][2], num[N], cnt[N];
	void init() {
		n=0; tot=1;
		len[1]=-1; fail[1]=0;
		len[0]=+0; fail[0]=1;
...
inline void up(int x, int y) {
	umax(t[x].Max[0], t[y].Max[0]);
	umin(t[x].Min[0], t[y].Min[0]);
	umax(t[x].Max[1], t[y].Max[1]);
	umin(t[x].Min[1], t[y].Min[1]);
}
...
// 添加一个点（注意此处的添加可能导致这棵树不平衡，慎用！）
void add(int k) {
	t[k].Max[0] = t[k].Min[0] = t[k].d[0];
	t[k].Max[1] = t[k].Min[1] = t[k].d[1];
	t[k].val = t[k].sum = 0;
	t[k].l = t[k].r = t[k].f = 0;
	if (!root) {
		root = k;
...
struct Treap {
    ...
    */	
	...
	int split(int u, int x, int y) { // if size is long long, remember here
		if (x > y) return 0;
		pushdown(u);
		if (x == 1 && y == node[u].size) return copy(u);
...
struct DTree {
	...
	void link(int x, int y) {
		...
		f[x] = y;
		access(x);
		// 如果需要维护子树和 val2, sumval2 ，这样是不够的。因为增加了虚边，所以需要修改 val2 值。将上面的三行代码替换为以下代码：
		// makeroot(x);
		// makeroot(y);
		// f[x] = y;
		// val2[y] += sumval2[x];
...
示例代码的状态转移方程：
$$f(x) = \sum\limits_{y = son(x)}f(y) + (\sum\limits_{y=son(x)}sz(y))^2 - (\sum\limits_{y=son(x)}sz(y)^2) + (\sum\limits_{y=son(x)}sz(y))$$
...
void dfs1(int x, int root) {
	if (root) g[x].erase(find(g[x].begin(), g[x].end(), root));
	LL s1 = 0, s2 = 0;
	LL sf = 0;
	uf[x] = 0;
	for (auto y : g[x]) {
...
void dfs2(int x, int root) {
	...
	for (auto y : g[x]) {
		dsz[y] = s1 + 1 - usz[y];
		df[y] = (sf - uf[y]) + (s1 - usz[y]) * (s1 - usz[y]) - (s2 - usz[y] * usz[y]) + (s1 - usz[y]);
		dfs2(y, x);
	}
	dp[x] = min(ans, sf + s1 * s1 - s2 + s1);
}
...
接下来一行输出 $n$ 个实数表示 $x_1, x_2, \cdots , x_n$ 。
...
LL calcSumS3()
{
	...
	for (int i = 1; i < vbnd; ++i) {
		ret += d3[i] * 4 * (g[i] - 1);
		// 取 c = 1, 3c + 1 = 4. g[i] - 1 的原因是除去 F(1).
	}
	return ret;
}
\end{lstlisting}

\subsubsection*{应用2 求素数的 $k$ 次方前缀和}

我们只需使用洲阁筛的 Part 1 ，计算出来的就是 $(\sqrt{n}, n]$ 中素书的 $k$ 次幂和。
...
long long calc(long long n) {
	for (vbnd = 1; 1ll * vbnd * vbnd <= n; ++vbnd);
	for (pbnd = 0; pbnd < top && 1ll * prime[pbnd] * prime[pbnd] <= n; ++pbnd);
	l0[0] = 0;
	for (int i = 1; i < vbnd; ++i)
		for (l0[i] = l0[i - 1]; l0[i] < top && 1ll * prime[l0[i]] * prime[l0[i]] <= i; ++l0[i]);
	L0[vbnd] = 0;
	for (int i = vbnd - 1; i >= 1; --i) {
		LL x = n / i;
		for (L0[i] = L0[i + 1]; L0[i] < top && 1ll * prime[L0[i]] * prime[L0[i]] <= x; ++L0[i]);
	}
	for (int i = 1; i < vbnd; ++i) {
		g0[i] = i;
		g1[i] = S1(i);
		...
		for (int j = 1; j < vbnd && i < L0[j]; ++j) {
			...
			if (y < vbnd) {
				if (i - l0[y] < 0) {
					...
					G0[j] -= g0[y] - (i - l0[y]);
					G1[j] -= 1ll * p * (g1[y] % mod - (sum1[i] - sum1[l0[y]]) % mod) % mod;
					G1[j] = ((G1[j] % mod) + mod) % mod;
				}
			} else {
				if (i - L0[n / y] < 0) {
					G0[j] -= G0[n / y];
					G1[j] -= 1ll * p * G1[n / y] % mod;
					if (G1[j] < 0) G1[j] += mod;
				} else {
					G0[j] -= G0[n / y] - (i - L0[n / y]);
					G1[j] -= 1ll * p * (G1[n / y] % mod - (sum1[i] - sum1[L0[n / y]]) % mod) % mod;
					G1[j] = ((G1[j] % mod) + mod) % mod;
				}
			}
		}
	...
	for (int i = 1; i < vbnd; ++i) {
		G0[i] -= pbnd - L0[i];
		G1[i] -= (sum1[pbnd] - sum1[L0[i]]) % mod;
		G1[i] = ((G1[i] % mod) + mod) % mod;
	}
	return G0[1] - 1 + pbnd; // 不超过 n 的素数个数
	return ((G1[1] + mod - 1) % mod + sum1[pbnd]) % mod; // 不超过 n 的素数和
	return ((g1[j] - 1 + sum1[l0[j]]) % mod + mod) % mod; // 不超过 j 的素数和
	return ((G1[n / j] - 1 + sum1[pbnd]) % mod + mod) % mod; // 不超过 n / j 的素数和
}
...
void insert(vector<line> &a, const line &b) { // 按k从小到大的顺序，开口向上
	if (a.size() && a.back().k == b.k) a.pop_back();
	while (a.size() >= 2) {
		int k = a.size();
		if (1ll * (a[k - 1].k - b.k) * (a[k - 1].b - a[k - 2].b)
			>= 1ll * (a[k - 1].b - b.b) * (a[k - 1].k - a[k - 2].k))
				a.pop_back();
		else
			break;
	}
...
struct poly {
	vector<mod> C;
	poly() {}
	explicit poly(const vector<mod> &C_) : C(C_) {}
	static const poly zero;
	inline int deg() const {
		return (int)C.size() - 1;
	}
	inline mod operator[](int x) const {
		return (x < 0 || x > deg()) ? mod(0) : C[x];
	}
	inline mod& operator[](int x) {
		if (x > deg()) {
			C.resize(x + 1);
		}
		return C[x];
	...
	inline friend poly operator +(const poly& a, const poly& b) {
		vector<mod> c(max(a.deg(), b.deg()) + 1);
	...
	inline poly primitive() const {
		if (isZero()) {
			return zero;
		}
		vector<mod> res(2 + deg());
		for (int i = 1; i <= 1 + deg(); i++) {
			res[i] = C[i - 1] / i;
		}
		return poly(res);
	}
	inline mod operator()(mod x) const {
		mod res = 0;
		for (int i = deg(); i >= 0; i--) {
			res = res * x + C[i];
		}
	...
	// Expand P(x+t).
	inline poly shift(int t) const {
		poly res;
		res[deg()];
		vector<mod> binomial(deg() + 1, 0);
		binomial[0] = 1;
		for (int i = 0; i <= deg(); i++) {
			mod cur = 1;
			for (int j = i; j >= 0; j--) {
...
\subsection{求数列中每个位置到前面任意位置的 gcd}

maxgcd[i] 表示从第 $i$ 个位置往前出发，每个 $gcd(a_j, \cdots, a_i)$ 值最靠后的位置（first 表示 gcd 值， second 表示位置）。
